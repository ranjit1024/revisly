...
const DashboardWidgetLight = () => {
  const [selectedId, setSelectedId] = useState(2); // Default to Marcus (Highest Score)
  
  const selectedCandidate = candidates.find(c => c.id === selectedId);
  const otherCandidates = candidates.filter(c => c.id !== selectedId);

  return (
    <div className="w-full h-full bg-white rounded-xl border border-slate-200 p-5 flex flex-col font-sans text-slate-900 shadow-sm">
...
// Sub-component for the featured metrics
